...
function topoSortTables(tableNames: string[], edges: Array<[string, string]>): string[] {
  ...
  if (out.length !== nodes.size) {
    // Graphe cyclique improbable sur SQLite FK, mais on retombe sur un ordre stable.
...
function getRow(db: Database.Database, table: string, pkCol: string, id: number): Record<string, unknown> | null {
  const row = db
    .prepare(`SELECT * FROM ${quoteIdentifier(table)} WHERE ${quoteIdentifier(pkCol)} = ?`)
...
export function mergeFlowlearnSqliteJsonDump(db: Database.Database, payloadRaw: unknown): JsonMergeResult {
  ...
  const remapRowValues = (table: string, row: Record<string, unknown>): Record<string, unknown> => {
    ...
    // Heuristiques métier (évite de dupliquer des référentiels stables)
  ...
  const insertOrResolveRow = (table: string, row: Record<string, unknown>): 'inserted' | 'skipped' | 'mapped' => {
