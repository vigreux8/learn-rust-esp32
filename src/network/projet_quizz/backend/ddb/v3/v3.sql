/* ces un ensemble de question qui un thématique en commun par exemple en statistique ces choisir qu'elle modèle statistique utilisé */
CREATE TABLE IF NOT EXISTS "sous-collections" (
...
/* regroupe des question qui on un lien étroit ensemble (quand je répond a mes quizz si je crée une question ces la question Parent qui ma aidée a crée la question enfant  */
...
CREATE TABLE IF NOT EXISTS "relation_sous-collections" (
	"id" INTEGER NOT NULL UNIQUE,
	"sous_collection_id" INTEGER NOT NULL,
	"question_id" INTEGER NOT NULL,
	PRIMARY KEY("id"),
	FOREIGN KEY ("sous_collection_id") REFERENCES "sous-collections"("id")
	ON UPDATE NO ACTION ON DELETE NO ACTION,
	FOREIGN KEY ("question_id") REFERENCES "quizz_question"("id")
	ON UPDATE NO ACTION ON DELETE NO ACTION
);
